...
- Ingredient compositions: Base ingredients can expand to component ingredients (nested allowed, cycle-safe). Verification checks allergens on expanded leaves; falls back to `Ingredient.default_allergens` when no active lot mapping. Seeded compositions for tortillas and mayo; inspect via `GET /v1/ingredients/{ingredient_id}`.
- Integrations/imports: Simple JSON import endpoints — Toast-style menu `POST /v1/integrations/toast/menu/import` (list of MenuItem), xtraCHEF ingredients `POST /v1/integrations/xtrachef/ingredients/import` (Ingredient + optional VendorProduct), xtraCHEF recipes `POST /v1/integrations/xtrachef/recipes/import` (Recipe with RecipeLine), xtraCHEF compositions `POST /v1/integrations/xtrachef/ingredient-compositions/import` (IngredientComposition). Status via `GET /v1/integrations/status` (counts + last_import timestamps including compositions). All stores remain in-memory.
...
- Demo flow (also in `app.py` header):
  ...
  5) Optional imports: replace seeded menu/recipes/ingredients/compositions with your own using the integration endpoints (Toast menu import, xtraCHEF ingredients/recipes/ingredient-compositions import), then check `GET /v1/integrations/status`.
  6) Inspect compositions: `GET /v1/ingredients/TORTILLA_FLOUR` to see expansion; set `DEBUG_EXPANSION=1` env var to log expansion at startup.
  7) UPC lookups: `GET /v1/lookup/upc/048001214101` (Hellmann’s UPC) or list providers via `GET /v1/providers`.
