...
- Menu & recipes: `/v1/menu` exposes a Chipotle-like item (chicken_burrito) with modifiers; recipe rules map modifiers to base ingredients (CHICKEN, TORTILLA_FLOUR/CORN, CHEESE, MAYO) with trace for auditing.
...
- Demo flow (also in `app.py` header):
  ...
  5) UPC lookups: `GET /v1/lookup/upc/048001214101` (Hellmann’s UPC) or list providers via `GET /v1/providers`.
